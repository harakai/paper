...
次の3つの制約をすべて満たす必要がある。
\begin{itemize}
    \item \ref{taioumodel}節でブラシ付きDCモータのモデルを対象とする
    \item サブシステムを用いたモデルではない
...
慣性部品のモジュール名と電源部品のモジュール名に、慣性部品と電源部品のモジュール名を指定する理由については、\ref{sub:csv_scan}節で述べる。
...
\begin{figure}[t]
	...
	\caption{慣性部品のモジュール名、電源部品のモジュール名に誤りがあった場合のエラー文の例}
...
配列voltageと配列currentを式(\ref{siki:in})に適用し、入力の値を算出する。算出した入力の値と、配列outputの要素を、式(\ref{siki:effi})に適用し、効率の値を格納する配列efficiencyに算出結果を格納する。
...
始動電流とは、モータの起動時に流れる大きな電流値である。モータの起動直後は逆起電力が発生するため、モータ・コイル部分にかかる電圧値が下がり、電流値も下がる。
したがって、配列currentの要素の最大値を始動電流とする。

\subsubsection{無負荷電流}
無負荷電流とは、モータに負荷がかかっていないときの電流値である。まず、配列torqueの中で、値が0である要素のインデックスXを取得する。
そして、配列currentのインデックスXの要素を無負荷電流とする。

\subsubsection{無負荷回転数}
無負荷回転数とは、モータに負荷がかかっていないときの回転数の値である。まず、配列torqueの中で、値が0である要素のインデックスXを取得する。
...
定格トルクとは、最大効率時のトルク値である。まず、配列efficiencyの中で、最大効率である要素のインデックスYを取得する。
...
\begin{enumerate}
    ...
    \item x軸に配列torqueを、y軸に配列speedを指定し、ライブラリmatplotlib(表\ref{tab:libr}参照)を用いてグラフを生成し、画像として保存する
    \item x軸に配列torqueを、y軸に配列efficiencyを指定し、ライブラリmatplotlib(表\ref{tab:libr}参照)を用いてグラフを生成し、画像として保存する
    \item x軸に配列torqueを、y軸に配列outputを指定し、ライブラリmatplotlib(表\ref{tab:libr}参照)を用いてグラフを生成し、画像として保存する
\end{enumerate}
上記の処理で生成した4つの特性グラフを、それぞれ図\ref{fig:current}、図\ref{fig:speed}、図\ref{fig:effi}、図\ref{fig:output}に示す。
